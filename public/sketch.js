...
function setup() {
  ...
  //gui=grafic user interface，管显示的？所以代码的次序和在界面上的顺序有关系
  ...
  handleSliderInputChange();//这个要放在上面这几个回调函数？后面才可以
...
function emitData(){
   socket.emit("drawing", {
    xpos: mouseX / width,
    ypos: mouseY / height,
    userR: red(r), //get the color channels
    userG: green(g),//把brushcolor换成设置的变量rgb
    userB: blue(b),
    userS: bSize/ width  // userS: bSize / width  //see what's it's like to scale to users window
    //用bSize是随缩放等比例，用bSize / width会缩放后画笔大小差不多（后面在onDrawingEvent里的circle的大小设置为userS*width）
  });
}


function onDrawingEvent(data){
  fill(data.userR,data.userG,data.userB);//这里用了在emitData设置的变量userR
